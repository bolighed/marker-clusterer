...
function popupWindowFactory() {
    ...
    return class PopupWindow extends google.maps.OverlayView {
        ...
        _panMap() {
            ...
            // edgeOffset
            const mib = this._getMapInnerBounds(), wb = this._anchor.getBoundingClientRect();
            let dx = 0, dy = 0;
            if (mib.left >= wb.left) {
                dx = wb.left - mib.left;
            }
        ...
        _getMapInnerBounds() {
            const mb = this.getMap().getDiv().getBoundingClientRect(), mib = {
